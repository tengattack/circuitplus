...
class circuit_ctl {
	...
	function on_feedback() {
		...
		$circuit_id = 0;
		if ($_GET['circuit'] && $_GET['circuit']['id']) {
			$circuit_id = intval($_GET['circuit']['id']);
		}
		//if (!$_G['uid'] || $_G['member']['groupid'] <= 1) {
		//	showmessage('permission_error', $circuit_id ? ('/circuit/'.$circuit_id) : '');
		//}
		if ($circuit_id && $_GET['formhash'] == $_G['formhash']) {
			$content = trim($_GET['content']);
			if ($content) {
				$content = addslashes($content);
				C::t('circuit_feedback')->insert(array(
					'user_id' => $_G['uid'],
				...
				showmessage('feedback_add_success', '/circuit/'.$circuit_id);
			} else {
				showmessage('feedback_add_error', '/circuit/'.$circuit_id);
			}
		} else {
			showmessage('feedback_add_error');
		}
	}
	...
	function on_get() {
		...
		if ($circuit) {
			...
			dheader('Access-Control-Allow-Origin: *');
			dheader('Access-Control-Allow-Methods: GET, POST');  
	...
	static function build_lgd_data(&$circuit) {
		$lgd = array(
			'circuit' => array(
