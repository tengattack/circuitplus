 ...
 *      SUSTC-ShenZhen-B
 *
 *      $Id: class/account.php 2013-06-14 09:10:00 Zhuoteng Peng $
 */
...
*/
	function on_login() {
		...
		if($_GET['formhash'] != $_G['formhash']) {
			...
			$urlsignin = '/user/signin' . ($_GET['redirect'] ? ('?redirect='.$_GET['redirect']) : '');

			$_GET['username'] = $_GET['user']['username'];
			$_GET['password'] = $_GET['user']['password'];

			if(!($_G['member_loginperm'] = logincheck($_GET['username']))) {
				showmessage('login_strike', $urlsignin);
			}
			
			$_G['uid'] = $_G['member']['uid'] = 0;
			$_G['username'] = $_G['member']['username'] = $_G['member']['password'] = '';
			if(!$_GET['username'] || $_GET['username'] != addslashes($_GET['username'])) {
				showmessage('profile_illegal', $urlsignin);
			}
			if(!$_GET['password'] || $_GET['password'] != addslashes($_GET['password'])) {
				showmessage('profile_passwd_illegal', $urlsignin);
			}
			...
			if($result['status'] > 0) {
				...
				/*if ($result['member']['emailstatus'] == 0) {
					showmessage('location_activation', '/user/');
				}*/
				...
				//if ($_GET['redirect']) $_GET['redirect'] = $_GET['redirect'];
				$location = $_GET['redirect'] ? $_GET['redirect'] : ($result['member']['emailstatus'] == 0 ? '/user/' : '/');
				...
				if($_G['member_loginperm'] > 1) {
					showmessage($fmsg, $urlsignin, array('loginperm' => $_G['member_loginperm'] - 1));
				} elseif($_G['member_loginperm'] == -1) {
					showmessage('login_password_invalid', $urlsignin);
				} else {
					showmessage('login_strike', $urlsignin);
				}
			}
	...
	function on_logout() {
		...
		if($_GET['formhash'] != $_G['formhash']) {
			showmessage('logout_failed', dreferer(), array('formhash' => FORMHASH, 'ucsynlogout' => $ucsynlogout));
		}
...
class register_ctl {
	...
	function on_register() {
		...
		if (($_GET['formhash'] != $_G['formhash']) || !intval($_GET['agree'])) {
			include template($this->template);
		}
		/*if(!submitcheck('regsubmit', 0, $seccodecheck, $secqaacheck)) {
			...
			if(!$activation)*/ {
				...
				if (ei_get_user(addslashes($username))) {
					if (ei_user_is_active($username)) {
						...
						showmessage('register_activation_message', '/user/signin/', array('username' => $username));
					}
				}
			...
			if($sendurl) {
				...
				$registerurl = "{$_G[siteurl]}user/activate?hash={$hashstr}&amp;uid={$uid}";
				...
				if(!sendmail("$_GET[email] <$_GET[email]>", lang('email', 'email_register_subject', array('username' => $username)), $email_register_message)) {
					runlog('sendmail', "$_GET[email] sendmail failed.");
				}
				//dreferer()
				showmessage('register_email_send_succeed', '/user/signin/', array('bbname' => $this->setting['bbname']), array('showdialog' => true, 'msgtype' => 3, 'closetime' => 10));
			}
...
class password_ctl {
	...
	function on_reset() {
		...
		if ($_GET['hash'] && $_GET['uid']) {
			...
			if (intval($hash[0]) != $uid || TIMESTAMP - $hash[2] >= 259200) {
				showmessage('user_info_error', '/user/password/reset/');
			}
			...
			$common_user = getuserbyuid($uid);
			$user['avatar_url'] = $common_user['avatar_url'];
			$user['email'] = $common_user['email'];

			$navtitle = lang('title', CURSCRIPT.'.password_input');
			include template('user/password_input');
		} else {
			
			$navtitle = lang('title', CURSCRIPT.'.password_reset');
			include template('user/password_reset');
		}
	}
	...
	function on_change() {
		global $_G;
		if (!$_G['uid']) {
			showmessage('permission_error', '/user/signin/');
		}
		
		$navtitle = lang('title', CURSCRIPT.'.password_change');
		include template('user/password_change');
	}
	...
	function on_submit() {
			...
			if ($_POST['formhash'] == $_G['formhash']) {
				if ($_GET['type'] == 'reset') {
					...
					if ($_GET['username'] && $_GET['email']) {
						...
						if ($user) {
							...
							if ($common_user && $common_user['email'] == $email) {
								...
								$registerurl = "{$_G[siteurl]}user/password/reset?hash={$hashstr}&amp;uid={$user[uid]}";
								...
								showmessage('password_reset_email_send_succeed', '/user/signin/');
							}
						}
						showmessage('user_info_error', '/user/password/reset/');
					}
					...
					if ($_GET['password1'] && $_GET['password2'] && $_GET['uid'] && $_GET['hash']) {
						...
						showmessage('reset_password_success', '/user/signin/');
					}
					showmessage('reset_password_error', '/user/password/reset/');
					...
					if ($_GET['password1'] && $_GET['password2'] && $_GET['oldpassword']) {
						...
						if (!phpbb_check_hash($_GET['oldpassword'], $account['password'])) {
							showmessage('profile_passwd_wrong', '/user/password/change/');
						}
						...
						clearcookies();
						showmessage('change_password_success', '/user/signin/');
					}
		...
		if (in_array($_GET['type'], array('reset', 'change'))) {
			//dheader('Location: /user/password/'.$_GET['type'].'/');
			dheader('Location: /user/settings/'.$_G['uid'].'/');
		} else {
			showmessage('undefined_action');
		}
	}
...
class profile_ctl {
	...
	function do_password_change() {
		...
		$ctl_obj->setting = $this->setting;
		$ctl_obj->user = & $this->user;
		return $ctl_obj->on_submit();
	}
	...
	function do_saveavatar() {
		...
		$upload = new bm_upload();
		if($upload->init($_FILES['avatar_file'], 'avatar') && $upload->save(1, $user['uid'])) {
	...
	function on_submit() {
		...
		$result = $this->on_edit();
		$get_user = $_GET['user'];
		if ($get_user['new_password'] && $get_user['new_password_again'] && $get_user['password']) {
			$_GET['password1'] = $get_user['new_password'];
			$_GET['password2'] = $get_user['new_password_again'];
			$_GET['oldpassword'] = $get_user['password'];
			$_GET['type'] = 'change';
			$result = $this->do_password_change();
		}
		dheader('Location: /user/settings/'.$this->user['uid'].'/');
		return $result;
	}
	...
	function on_edit() {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			if ($_POST['formhash'] == $_G['formhash']) {
				...
				$up = array (
					'realname' => trim($_GET['user']['realname']),
					'telephone' => trim($_GET['user']['phonenumber']),
					//'mobile' => trim($_GET['user']['mobile']),
					//'qq' => trim($_GET['user']['qq']),
					//'msn' => trim($_GET['user']['msn']),
					'site' => trim($_GET['user']['site']),
					'bio' => trim($_GET['user']['bio']),
					'gender' => intval($_GET['user']['gender']),
				);
				...
				if ($up['site']) {
					if (strtolower(substr($up['site'], 0, 7)) != 'http://' && strtolower(substr($up['site'], 0, 8)) != 'https://') {
				...
				if (!$this->nolocation) {
					dheader('Location: /user/settings/'.$user['uid'].'/');
				}
			}
		}
		...
		if (!$u) showmessage('message_bad_touid', '/user/settings/'.$user['uid'].'/');
	...
	function on_antiactivate_email() {
		...
		if ($_GET['formhash'] != $_G['formhash']) {
			dheader('Location: /user/'.$user['uid'].'/');
		}
		...
		if ($u['emailstatus'] != 1) {
			showmessage('email_is_not_activated', '/user/'.$user['uid'].'/');
		}
		...
		//dreferer()
		showmessage('antiactivate_email_send_succeed', '/user/'.$user['uid'].'/', array('bbname' => $this->setting['bbname']), array('showdialog' => true, 'msgtype' => 3, 'closetime' => 10));
	}
	...
	function on_resend_email() {
		...
		if ($u['emailstatus'] == 1) {
			showmessage('email_is_activated', '/user/'.$user['uid'].'/');
		}
		...
		if(!sendmail("$_GET[email] <$_GET[email]>", lang('email', 'email_register_subject', array('username' => $u['username'])), $email_register_message)) {
			runlog('sendmail', "$_GET[email] sendmail failed.");
		}
		//dreferer()
		showmessage('register_email_send_succeed', '/user/'.$user['uid'].'/', array('bbname' => $this->setting['bbname']), array('showdialog' => true, 'msgtype' => 3, 'closetime' => 10));
	}
				...
				if ($_GET['password'] && $_GET['email']) {
					...
					if ($email) {
						...
						if (!phpbb_check_hash($_GET['password'], $account['password'])) {
							showmessage('profile_passwd_wrong', '/user/'.$user['uid'].'/change_email/');
						}
						$u = getuserbyuid($uid);
						if (!$u) {
							showmessage('message_bad_touid', '/');
						...
						if ($u['emailstatus'] == 1) {
							showmessage('email_is_activated', '/user/'.$user['uid'].'/change_email/');
						}
