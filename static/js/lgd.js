...
function init_lgd_var(circuit_lgd) {
  ...
  for (var f=0;f<coding_num;f++) {
    for(var e=0;e<circuit_lgd.codingframe[f].biobrick.length;e++) {
      cf[d]=parseInt(circuit_lgd.codingframe[f].biobrick[e].dnaproperty.id)-3;  /*具体哪个不清楚*/
  ...
  for (var e=0;e<circuit_lgd.substance.length;e++) {
    ...
    substance_id[e]=parseInt(circuit_lgd.substance[e].id);
  }
  ...
  for (var e=0; e<circuit_lgd.regulation.length;e++) {
    var rtype = parseInt(circuit_lgd.regulation[e].type);
    switch (rtype) {
    case 0:
      source_2[a]=parseInt(circuit_lgd.regulation[e].source.id)-f+1;
      target_2[a]=parseInt(circuit_lgd.regulation[e].target.id)-f+1;
      relatype_1[a]=parseInt(circuit_lgd.regulation[e].relation.id);
      expression[a]=parseInt(circuit_lgd.regulation[e].expression.id)-f+1;    
      a++;
      break;
    case 1:
      source_3[b]=parseInt(circuit_lgd.regulation[e].source.id)-f+1;
      target_3[b]=parseInt(circuit_lgd.regulation[e].target.order)+each_length[parseInt(circuit_lgd.regulation[e].target.codingframe_id)-1];
      relatype1[b]=parseInt(circuit_lgd.regulation[e].relation.id)-3;
      ...
      break;
    case 2:
      source_1[c]=parseInt(circuit_lgd.regulation[e].source.order)+each_length[parseInt(circuit_lgd.regulation[e].source.codingframe_id)-1];
      target_1[c]=parseInt(circuit_lgd.regulation[e].expression.id)-f+1;
      c++;
      break;
    }
  }
